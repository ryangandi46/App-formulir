...
// Base URL (sesuaikan nama folder kalau beda)
...
<head>
    ...
    <style>
        ...
        .link-input { font-size:.8rem; }
        .badge-upload { font-size:.7rem; }
    </style>
</head>
<body>
    ...
    <div class="card card-g shadow-sm">
        <div class="card-header-g d-flex justify-content-between align-items-center">
            <div>
                ...
                <small>Kelola form, link publik, QR, detail, dan responses.</small>
            </div>
            <div class="d-flex gap-2">
                <a href="create_form.php" class="btn btn-light btn-sm">+ Buat Form Baru</a>
                <a href="../logout.php" class="btn btn-outline-light btn-sm">Logout</a>
            </div>
        </div>
        <div class="card-body">
            <?php if (empty($forms)): ?>
                <div class="alert alert-info mb-0">Belum ada form.</div>
            <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="input-group input-group-sm">
                                        <input id="link<?= $id ?>" type="text" class="form-control link-input"
                                               value="<?= $publicLink ?>" readonly>
                                        <button class="btn btn-outline-primary" type="button"
                                                onclick="copyLink('<?= $id ?>')">Salin</button>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm" role="group">
                                        <a href="view_form.php?key=<?= $key ?>" class="btn btn-outline-secondary">Detail</a>
                                        <a href="view_response.php?key=<?= $key ?>" class="btn btn-outline-info">Responses</a>
                                        <a href="edit_form.php?key=<?= $key ?>" class="btn btn-outline-primary">Edit</a>
                                        <a href="delete_form.php?key=<?= $key ?>" class="btn btn-outline-danger"
                                           onclick="return confirm('Yakin hapus form ini?');">Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary btn-sm" onclick="openQRNewTab()">Buka di Tab Baru</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function showQR(src, title) {
        ...
        let m = new bootstrap.Modal(document.getElementById('qrModal'));
        m.show();
    }
    function openQRNewTab() {
        if (qrSrcGlobal) window.open(qrSrcGlobal, "_blank");
    }
</script>
</body>
</html>

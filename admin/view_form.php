...
// Bisa akses pakai ?key=... (baru) atau ?id=... (fallback)
if (isset($_GET['key'])) {
    $key = $_GET['key'];
...
$publicLink = $baseUrl . "/f/" . $form['public_key'];
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        .q-type-badge { font-size:.7rem; }
        .options-box {background:#f8f9fa;border-radius:8px;padding:8px 10px;font-size:.85rem;}
    ...
    <div class="card card-g shadow-sm">
        <div class="card-header-g d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-2">
                <a href="dashboard.php" class="btn btn-light btn-sm">&larr; Kembali</a>
                <a href="view_response.php?key=<?= $form['public_key'] ?>" class="btn btn-outline-light btn-sm">Responses</a>
                <a href="edit_form.php?key=<?= $form['public_key'] ?>" class="btn btn-outline-light btn-sm">Edit Form</a>
            </div>
        </div>
        <div class="card-body">
            <div class="mb-4">
                ...
                <?php if (!empty($form['description'])): ?>
                    <p class="mb-2 text-muted"><?= nl2br(htmlspecialchars($form['description'])) ?></p>
                <?php endif; ?>
                ...
                <div class="mb-2">
                    ...
                    <div class="input-group input-group-sm">
                        <input id="publicLink" type="text" class="form-control link-input"
                               value="<?= $publicLink ?>" readonly>
                        <button class="btn btn-outline-primary" type="button" onclick="copyPublicLink()">Salin</button>
                        <a href="<?= $publicLink ?>" target="_blank" class="btn btn-outline-success">Buka Form</a>
                    </div>
                </div>
            </div>
            ...
            <?php else: ?>
                <?php foreach ($questions as $idx => $qrow): ?>
                    <div class="card question-card shadow-sm mb-3">
                        <div class="card-body">
                            <div class="d-flex justify-content-between mb-1">
                                <div class="fw-semibold">
                                    <?= ($idx + 1) . '. ' . htmlspecialchars($qrow['question']); ?>
                                </div>
                                <div>
